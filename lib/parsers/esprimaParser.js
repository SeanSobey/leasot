...
module.exports = function (params) {
    ...
    return function parse(contents, file) {
        ...
        var comments = [];
        extract(contents).forEach(function (comment) {
            var match = regex.exec(comment.value);
            while (match) {
                var line = comment.loc.start.line + commentsUtil.getLineFromPos(comment.value, match.index) - 1;
